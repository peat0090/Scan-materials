        ...
        {!active && (
          <div className="text-center p-8">
            ...
            <p className="text-slate-400 text-sm mb-4">
              {loaded ? 'กดปุ่มด้านล่างเพื่อเปิดกล้องสแกน QR Code' : 'กำลังโหลด scanner...'}
            </p>
            <button
              onClick={startCamera}
              disabled={!loaded}
        ...
        {active && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="relative w-52 h-52">
              ...
              {scanning && (
                <div className="absolute inset-x-0 h-0.5 bg-white/80 scan-line" />
              )}
            </div>
          </div>
        )}
  ...
  const handleQRScan = (raw) => {
    try {
      ...
      setForm(prev => ({
        ...
        section:      obj.section  || prev.section,
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        <div className="mb-6">
          ...
          <p className="text-slate-500 text-sm mt-0.5">รองรับ iOS Safari / Android / Chrome</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white/5 border border-white/10 rounded-2xl p-5 space-y-4 fade-in">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-xs text-slate-400 uppercase tracking-widest block mb-1.5">ID สินค้า <span className="text-red-400">*</span></label>
              <input value={form.product_id} onChange={e => set('product_id', e.target.value)}
                placeholder="เช่น SKU-0001 หรือจากการสแกน" required
